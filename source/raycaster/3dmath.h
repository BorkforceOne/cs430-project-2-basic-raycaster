...
static inline void v3_dot(V3 *a, V3 *b, double *dotResult) {
	*dotResult = a->X * b->X + a->Y * b->Y + a->Z * b->Z;
}
...
static inline void v3_magnitude(V3 *a, double *magnitudeResult) {
	*magnitudeResult = sqrt(a->X*a->X + a->Y*a->Y + a->Z*a->Z);
}
